...
static inline void spin_lock(spinlock_t *lock) {
    while (__sync_lock_test_and_set(&lock->locked, 1)) {
        /* Busy wait with pause instruction if available */
...
static spinlock_t early_alloc_lock = SPINLOCK_INIT;
#define EARLY_LOCK() spin_lock(&early_alloc_lock)
#define EARLY_UNLOCK() spin_unlock(&early_alloc_lock)
#else
